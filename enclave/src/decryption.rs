...
impl<R: Read> EncryptedReader<R> {
    pub fn with_capacity(capacity: usize, inner: R, key_bytes: &[u8]) -> Self {
        Self {
            inner: Some(inner),
    ...
    fn fill_buf(&mut self) -> Result<()>{
        ...
        let r = self.inner.as_mut().unwrap().read_u32::<NetworkEndian>();
        ...
        while read < nonce.len() {
            let r = self.inner.as_mut().unwrap().read(&mut nonce[read..]);
            match r {
                Ok(0) => {
        ...
        let mut read = 0;
        while read < len {
            let r = self.inner.as_mut().unwrap().read(&mut self.buf[read..]);
            match r {
                Ok(0) => {
                    return Err(Error::new(ErrorKind::UnexpectedEof,
                                          "Failed to read"));
